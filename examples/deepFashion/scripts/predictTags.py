...
def CreateClassifier(settings_file):
	args=getSetings(settings_file)
...
def InputImagePredict(input_file,settings_file,mode,classifier):
	...
	if mode=="embedding":	
		...
		result=list()
		for i in range(predictions.shape[0]):
			localpredictions=predictions[i]
			print sum(localpredictions)
			label=localpredictions[:10]
			color=localpredictions[10:26]
			fabric=localpredictions[26:47]
			style=localpredictions[47:55]
			print np.sum(label)
			print np.sum(color)
			print np.sum(fabric)
			print np.sum(style)
